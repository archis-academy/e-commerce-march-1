...
async function DisplayProducts() {
              ...
              <div class="best-selling-img-container">
                <img class="best-selling-products-img" src="${product.image}"/>
                 <div class="best-selling-products-icons-container">
                  ...
                  <div onclick="addToCard(${product.id})" class="selling-card-icon-container">
                    <svg width="32" height="32" id="card_${product.id}" class="selling-card-icon" viewBox="0 0 32 32" fill="none" xmlns="http://www.w3.org/2000/svg">
  ...
  colorizedWishlistProducts.map((product) => {
    ...
    console.log(wishlistSvgs);
    wishlistSvgs.style.fill = "crimson";
   });

   const cardButtons = sellingProductsContainer.querySelectorAll(".selling-card-icon");
   
   cardButtons.forEach((button) => {
    let buttonClicked = true;
    
    button.addEventListener(("click"), () => {
      if(buttonClicked && button.style.fill !== "dimgray") {
        button.style.fill = "dimgray";
        buttonClicked = false;
      }
      else {
        button.style.fill = "none";
  ...
  colorizedCardProducts.map((product) => {
    const cardSvgs = sellingProductsContainer.querySelector(`#card_${product.id}`);
    console.log(cardSvgs);
    cardSvgs.style.fill = "dimgray";
  });
};
