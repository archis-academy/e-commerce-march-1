...
async function DisplayBestSellingProducts() {
  ...
  sellingProductsContainer.innerHTML = bestSellingProducts
    .map((product) => {
                ...
                <img class="best-selling-products-img" src="${product.image}"/>
                 <div class="best-selling-products-icons-container">
                  <div onclick="addToWishlist(${
                    product.id
                  })"  class="selling-wishlist-icon">                
                   <svg width="32" id="wishlist_${
                     product.id
                   }" class="selling-products-wishlist-svg" height="32" viewBox="0 0 32 32" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path d="M11 7C8.239 7 6 9.216 6 11.95C6 14.157 6.875 19.395 15.488 24.69C15.6423 24.7839 15.8194 24.8335 16 24.8335C16.1806 24.8335 16.3577 24.7839 16.512 24.69C25.125 19.395 26 14.157 26 11.95C26 9.216 23.761 7 21 7C18.239 7 16 10 16 10C16 10 13.761 7 11 7Z" stroke="black" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
                   </svg>  
                 </div> 
                 <div  onclick="addToCart(${product.id})"  id="cart_${
        product.id
               ...
               <div class="best-selling-products-features">
                 ...
                 <div class="bestselling-rating-container">
                   ...
                   <div class="bestselling-transparent-div" id="transparent-div" style="width:${hideStars(
                     product
                   )}%"></div>   
                   <p class="best-selling-rating">(${
  ...
  colorizedWishlistProducts.map((product) => {
    const wishlistSvgs = sellingProductsContainer.querySelector(
      `#wishlist_${product.id}`
    );
    wishlistSvgs.style.fill = "crimson";
  });

  const wishlistButtons = sellingProductsContainer.querySelectorAll(
    ".selling-products-wishlist-svg"
  );
  ...
  colorizedCartProducts.map((product) => {
    const cartDivs = sellingProductsContainer.querySelector(
      `#cart_${product.id}`
    );
    const checkArray = bestSellingProducts.some(
  ...
  const cartContainer = sellingProductsContainer.querySelectorAll(
...
function addToWishlist(productId) {
  ...
  const clickedProduct = bestSellingProducts.find(
...
function addToCart(productId) {
  ...
  const clickedForCart = bestSellingProducts.find(
    (product) => product.id === productId
  );
